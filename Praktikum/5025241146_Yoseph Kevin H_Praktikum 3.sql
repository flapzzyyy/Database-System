...
-- no 2
SELECT p.nama_pelanggan, o.jumlah_kamar, GROUP_CONCAT(k.nama_kamar ORDER BY k.nama_kamar) as nama_kamar FROM pelanggan p
...
GROUP BY p.nama_pelanggan, o.jumlah_kamar;

-- no 3
SELECT r.nama_resepsionis, COUNT(1) AS total_order
FROM resepsionis r
JOIN `order` o ON r.id = o.resepsionis_id
JOIN order_kamar ok ON o.id = ok.order_id
GROUP BY r.id
HAVING COUNT(ok.kamar_nomor) > 2;
...
-- no 5
SELECT k.nomor, k.nama_kamar, k.tipe_kamar FROM kamar k
JOIN kamar_fasilitas kf ON k.nomor = kf.kamar_nomor
JOIN fasilitas f ON kf.fasilitas_id = f.id
WHERE f.nama = 'Wi-Fi';

-- no 6
SELECT SUM(DATEDIFF(o.tanggal_checkout, o.tanggal_checkin) * k.harga_per_malam) AS total, 
AVG(DATEDIFF(o.tanggal_checkout, o.tanggal_checkin) * k.harga_per_malam) AS rata_rata FROM `order` o
JOIN order_kamar ok ON o.id = ok.order_id
JOIN kamar k ON ok.kamar_nomor = k.nomor;

-- no 7
SELECT p.NIK, p.nama_pelanggan, p.no_telp, k.nomor, k.nama_kamar, k.tipe_kamar FROM pelanggan p
JOIN `order` o ON p.NIK = o.pelanggan_NIK
JOIN pembayaran b ON o.id = b.order_id
JOIN order_kamar ok ON o.id = ok.order_id
JOIN kamar k ON ok.kamar_nomor = k.nomor
WHERE b.`status` = 'Belum Lunas';

-- no 8
SELECT k.nomor, k.nama_kamar, GROUP_CONCAT(f.nama ORDER BY f.nama) AS nama_fasilitas FROM kamar k
JOIN kamar_fasilitas kf ON k.nomor = kf.kamar_nomor
JOIN fasilitas f ON kf.fasilitas_id = f.id
WHERE k.tipe_kamar = 'Deluxe'
GROUP BY k.nomor, k.nama_kamar;

-- no 9
SELECT f.nama, COUNT(1) AS jumlah_penggunaan FROM fasilitas f
JOIN kamar_fasilitas kf ON f.id = kf.fasilitas_id
GROUP BY f.nama
